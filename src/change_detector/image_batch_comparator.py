...
class ImageBatchComparator:
    ...
    def join_images(
        self, image1_filepath, image2_filepath, outfilepath, text=None, changed=None
    ):
        ...
        if img1_height != img2_height:
            img2 = img2.resize(
                (int(img2.size[0] * img1_height / img2_height), img1_height)
            )
...
if __name__ == "__main__":
    ...
    for i in range(len(files)):
        for j in range(i + 1, len(files)):
            out_filepath = (
                generated_images_out_dir + "analysis_" + str(i) + "x" + str(j) + ".jpeg"
            )
            ...
            app.join_images(
                files[i], files[j], out_filepath, text=sim_text, changed=changed_image
            )
