    ...
    async def send_image(self, image_path: Path | str, **kwargs) -> Response:
        ...
        try:
            with open(image_path, "rb") as image_file:
                files = {"file": (str(image_path), image_file, "image/jpeg")}

                data = {"content": f"{format_users(CONFIG.discord.listening_users)}"}

                response = requests.post(webhook, files=files, data=data)

            if response.status_code != 200:
                logger.error(
                    f"Unexpected status code {response.status_code} when sending message to Discord"
                )
                return Response(
                    False,
                    f"Unexpected status code {response.status_code} when sending message to Discord",
